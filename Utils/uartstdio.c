...
//            if(!RX_BUFFER_FULL)
        ...
        char uart_led_cmd = 3;
        if (g_pcUARTRxBuffer[g_ui32UARTRxWriteIndex - 1] == 'F'
        	&& g_pcUARTRxBuffer[g_ui32UARTRxWriteIndex - 2] == 'L')
        {
        	if (g_pcUARTRxBuffer[g_ui32UARTRxWriteIndex - 7] == 'N')
        		uart_led_cmd = 0;
        	else if (g_pcUARTRxBuffer[g_ui32UARTRxWriteIndex - 7] == 'F')
        		uart_led_cmd = 1;
        	else if (g_pcUARTRxBuffer[g_ui32UARTRxWriteIndex - 7] == 'E')
        		uart_led_cmd = 2;
        }
		...
		if (uart_led_cmd != 3)
		{
			...
			if (xQueueSendFromISR(xQueue, &uart_led_cmd, &xHigherPriorityTaskWoken ) != pdPASS)
				UARTprintf( "USART_ISR: Could not send to the queue.\r\n" );
		}
    }
}
